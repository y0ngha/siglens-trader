...
function sectionAccount(input: TradeGateInput): string[] {
    ...
    // 브로커에서 오지만, 모델이 이 값으로 하는 일은 하나다 — "지금 쓸 수 있는 돈이 얼마인가".
    // 출처는 바로 위 `매매 모드` 줄이 이미 말하고 있고, 모드별로 문구를 갈라 두면 같은 결정에
    // 서로 다른 사이징 습관이 붙는다. dry_run 잔고도 실제로 `planEntry`를 제약하므로
    // 시스템 규칙 2("적힌 값은 참")를 어기지 않는다.
    const cashLine =
        input.kind === 'entry'
            ? `- 매수 가능 현금: ${
                  a.availableCashUsd === null
                      ? `미상 (브로커 잔고 조회 실패. 이 불확실성 자체를 보수적 요인으로 취급하라)`
