...
function sectionPosition(input: TradeGateInput): string[] {
    ...
    return [
        ...
        // 진입 결정에서만 방향을 못박는다. 모델이 평단과 현재가로 계산해 낼 수는 있지만,
        // **예산이 커진 이유**는 계산으로 나오지 않는다: 종목당 예산은
        // `한도 − 현재가 × 보유수량`이라 가격이 내릴수록 커진다. 그 증가분은 기회가 커진 것이
        // 아니라 평가액이 줄어든 것이며, 이 문장이 없으면 `## 예산`의 큰 숫자가 그대로
        // "더 살 수 있다"는 신호로 읽힌다.
        ...(input.kind === 'entry' && avg !== null
            ? [
                  input.price < avg
                      ? `- 이번 결정의 성격: **평단 아래 추가 매수(물타기)**. 이 종목의 집행 가능 예산은 \`한도 − 현재가 × 보유수량\`이라 가격이 내려간 만큼 자동으로 커져 있다 — 예산 증가는 기회의 증가가 아니라 평가액 감소의 결과다. 또한 고정 손절선을 쓰는 경우 그 기준은 평단이므로, 추가 매수는 손절선을 함께 아래로 옮긴다.`
...
const GUIDELINES: Record<TradeGateKind, string[]> = {
    entry: [
        ...
        '5. **기존 포지션과 추가 매수의 방향.** 이미 종목당 한도의 상당 부분을 채웠다면 추가 매수는 작아야 한다. ' +
            '`## 포지션`에 성격이 적혀 있으면 그것을 사이징에 반영한다 — 물타기는 예산이 커져 보이는 것이 ' +
            '가격 하락의 결과이므로 그 증가분을 근거로 크기를 키우지 않는다. 다만 축이 일치하고 진입 근거가 ' +
            '살아 있다면 물타기 자체를 금지하지는 않는다. 판단의 근거는 예산의 크기가 아니라 분석이다.',
