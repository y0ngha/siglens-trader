...
export interface RiskRewardLevels {
    ...
    resistance?: number;
    /** `priceTargets.bullish` 첫 목표 — 규칙 5b가 95% 지점에서 발동한다(`takeProfit` 부재 시에만). */
    target?: number;
 ...
 *
 * **저항선과 목표가는 `takeProfit`이 없을 때만 후보다.** 규칙 5·5b가 4.5의 폴백이라
 * (그쪽 주석 참고) 분석 익절가가 있으면 둘 다 아예 발동하지 않는다. 여기서 세면 청산 체인이 서지도
...
export function firstUpsideExit(price: number, levels: RiskRewardLevels): number | null {
    ...
    // 규칙 5(저항 밴드)와 5b(목표가 95%)는 둘 다 4.5의 폴백이다 — 분석 익절가가 있으면
    // 서지 않는다.
    const fallbackCounts = !isFinitePositive(levels.takeProfit);
    const candidates = [
        levels.takeProfit,
        fallbackCounts && isFinitePositive(levels.resistance)
            ? levels.resistance * RESISTANCE_TRIGGER_RATIO
            : undefined,
        fallbackCounts && isFinitePositive(levels.target)
            ? levels.target * TARGET_TRIGGER_RATIO
            : undefined,
