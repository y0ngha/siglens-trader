...
describe('isExecuteInterval', () => {
    ...
    it('15·20·30·60분은 거부한다 — 마감 20분 창에 재시도 틱이 없다', () => {
        for (const v of [15, 20, 30, 60]) expect(isExecuteInterval(v)).toBe(false);
    });
...
describe('parseExecuteInterval', () => {
    it('유효한 값은 그대로 돌려준다', () => {
        ...
        expect(parseExecuteInterval(10)).toBe(10);
    });

    it('손상된 값은 기본값으로 떨어진다', () => {
        for (const v of [null, undefined, 'abc', 7, 60, NaN, {}]) {
...
describe('isExecuteTick', () => {
    ...
    it('모든 허용 간격이 마감 20분 창(:40~:59)에 틱을 둘 이상 둔다 — 재시도 여지', () => {
        for (const interval of EXECUTE_INTERVALS) {
            expect(firingMinutes(interval).filter((m) => m >= 40).length).toBeGreaterThanOrEqual(2);
        }
    });
    ...
    it('기준 분 직전(:03~:05)은 10분 설정에서 실행되지 않는다', () => {
        for (const m of [3, 4, 5]) expect(isExecuteTick(at(m), 10)).toBe(false);
    });

    it('초·밀리초는 판정에 영향을 주지 않는다', () => {
        expect(isExecuteTick(new Date(Date.UTC(2026, 7, 17, 15, 7, 59, 999)), 10)).toBe(true);
    });
});
