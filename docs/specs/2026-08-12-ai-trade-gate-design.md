...
`fraction = 0`(보류)에는 상한을 두지 않는다. 모델이 청산을 계속 미룰 수 있다는 뜻이며, 이때의
최종 안전망은 기존 서킷 브레이커 — **킬 스위치**와 **일일 손실 한도(미실현 포함)** 다. 후자는
미실현 손실까지 합산해 한도를 넘으면 매매 전체를 중지시키므로, 무한 보류의 손실은 그 한도에서
잘린다. (운영자가 이 트레이드오프를 명시적으로 선택했다.)
...
| `execute.test.ts` | 분할 진입(fraction 0.5 → 절반 수량), 부분 청산 → `reducePositionQuantity`, 전량 청산 → `closePosition`, 진입 게이트 실패 → 주문 없음 + 메일, 청산 게이트 실패 → 전량 매도, `hard` 손절 → 게이트 미호출, `entry_deferred`/`exit_deferred`, 게이트 OFF → 기존 동작 동일 |
...
게이트를 `설정 > 분석 설정 > 매매 게이트`에서 OFF로 돌리면 즉시 배포 이전 동작(전량 진입 /
전량 청산)으로 복귀한다. 코드 롤백이나 재배포가 필요 없다.
