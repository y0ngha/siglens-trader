...
- **`entry_not_recommended`** — 분석의 `entryRecommendation`이 `avoid`면 점수와 무관하게 매수를
  ...
  검사로는 걸러지지 않는다.
- **`average_down_blocked`** — 평단 아래 추가매수는 기본 차단(`average_down_enabled`, 기본 off).
  종목 예산이 `현재가 × 보유수량`을 빼는 시가 기준이라 **가격이 내릴수록 살 수 있는 금액이
  커지고**, 고정 손절선은 평단 기준이라 물타기가 손절선을 함께 끌어내린다.
