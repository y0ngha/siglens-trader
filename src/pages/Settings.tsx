...
const ANALYSIS_TYPES = ['technical', 'news', 'fundamental', 'entry_review'] as const;
...
export function SettingsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* 전략 설정 — 일봉 RSI(2) 눌림매수 파라미터
                (docs/specs/2026-09-24-daily-mean-reversion-design.md §3, §6) */}
            <section className="rounded-lg border border-[#262626] bg-[#141414] p-4">
                <h2 className="text-sm font-semibold">전략 설정</h2>
                <p className="mt-0.5 text-[10px] text-neutral-600">
                    일봉 RSI(2) 눌림매수 — 200일선 위에서 단기 과매도를 사고, 5일선 회복·보유
                    기간·재난 손절로 판다
                </p>
                    ...
                    {(
                        [
                            ...
                            [
                                'mr_stop_atr',
                                '재난 손절 ATR 배수',
                                '진입가 − 배수 × ATR(14)에 손절. 0이면 손절 없음 (기본 5)',
                                0,
                                20,
                                0.5,
                            ],
                ...
                <div className="mt-3 flex items-center justify-between rounded border border-[#262626] bg-[#0a0a0a] p-3">
                    <div>
                        ...
                        <p className="text-[10px] text-neutral-600">
                            ON이면 SPY가 200일선 아래인 동안 신규 매수를 하지 않습니다
                        </p>
                    </div>
                    <button
                        type="button"
                        ...
                        aria-label={`시장 국면 필터 ${mrRegimeFilter ? '비활성화' : '활성화'}`}
                    >
                        {mrRegimeFilter ? 'ON' : 'OFF'}
                    </button>
                </div>
            </section>

            {/* 매매 실행 주기 — 손절가 확인 간격. 매수·청산 판단은 하루 한 번, 장 마감 20분
                전 창의 첫 틱에서만 한다. 선택지는 lib/strategy/execute-interval.ts의
                EXECUTE_INTERVALS와 같아야 한다(src/는 lib/를 import하지 않는 것이 이 저장소
                규칙이라 값을 복제한다) — 창 안에 재시도 틱이 최소 하나 있어야 해서 5·10분뿐이다. */}
            <section className="rounded-lg border border-[#262626] bg-[#141414] p-4">
                ...
                <p className="mt-2 text-[11px] leading-relaxed text-neutral-500">
                    손절가 도달을 확인하는 간격입니다. 매수·청산 판단은 장 마감 20분 전에 하루 한
                    번만 합니다.
                </p>
            </section>
            ...
            <section className="rounded-lg border border-[#262626] bg-[#141414] p-4">
                ...
                <div className="mt-3">
                    <TickerSearch
                        onSelect={(result) => {
                            if (configData.watchlist.length >= MAX_WATCHLIST_SIZE) {
                                setSaveMessage(
                                    `감시 종목은 최대 ${MAX_WATCHLIST_SIZE}개까지 설정 가능합니다`,
                                );
                                return;
                            }
                ...
                <ul className="mt-3 space-y-3">
                    {(configData.analysis.length > 0
                        ...
                        >
                            ...
                            {ac.analysisType === 'entry_review' && (
                                <p className="mt-0.5 text-[10px] text-neutral-600">
                                    신호가 난 종목의 하락 원인을 판단해 기록만 합니다 — 주문에는
                                    영향을 주지 않습니다
                                </p>
                            )}
                            <div className="mt-2 flex items-center gap-2">
            ...
            {/* Save button for all risk/investment/strategy changes */}
            {hasRiskChanges && (
                <div className="flex items-center gap-2">
                    <button
                        type="button"
