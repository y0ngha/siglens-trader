...
- 프롬프트에 **"이 축은 LLM 판단이 아니라 백테스트 승률 70% 규칙의 결정론적 출력이며,
  다른 축과 충돌할 때 더 무겁게 취급하라"**는 한 줄을 함께 렌더한다.
...
- **끄는 법**: 대시보드/`POST /api/config` 로 `score_weights.confluence = 0`.
  재배포 없이 즉시 축이 침묵한다. 별도 플래그를 만들지 않는 이유 — 이미 있는 노브로 충분하다.
- **가중치 조정**: 같은 경로. 저장된 `score_weights`가 타임프레임 프로파일을 키 단위로 덮어쓴다.
