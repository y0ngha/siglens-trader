...
- **노출 한도를 원가 기준으로 바꿨다.** `existingSymbolExposure` / `currentExposure`가
  `현재가 × 수량`(평가액)이었다. 그러면 가격이 내릴수록 남은 예산이 커진다 — 한도 $1,000에
  $100로 10주를 산 뒤 주가가 $50이 되면 평가액 $500 → "예산 $500 남음"이 되어 10주를 더 살 수
  있고, $25에서 반복하면 한도 $1,000짜리 종목에 원가 $2,000 이상이 들어갔다. 한도가 실제로
  아무것도 한정하지 못했고, 설정 라벨("종목당 최대 **투자 금액**")과도 어긋났다.
  `api/CLAUDE.md`가 "deliberately unchanged"로 적어 둔 것이지만, 그 문장은 이 결과를 계산해
  보지 않은 채 쓰였다. 부수 효과로 노출 계산 루프의 시세 조회가 통째로 사라진다 — 원가는 DB에
  이미 있다. 청산도 판 가격이 아니라 그 주식의 원가만큼 노출을 줄인다. 미실현 손익 차단기는
  평가액을 봐야 하므로 그대로 실시간 시세를 쓴다.
- **물타기는 게이트에 위임한다.** 감사는 "규칙 엔진이 방향을 구분하지 않는다"를 지적했고 초안은
  하드 차단(`average_down_enabled`, 기본 off)으로 갔는데, 그건 층을 잘못 고른 것이다 — 점수
  ≥70 + 6축 합의 + 사이징 게이트를 통과했다면 그것이 이미 AI의 추천이다. 차단을 걷어내고
  **게이트 프롬프트가 물타기임을 알도록** 바꿨다. 핵심은 모델이 계산으로 얻을 수 없는 사실이다:
  **고정 손절선의 기준이 평단**이라 추가 매수가 손절선을 함께 내리고, 같은 하락폭에서 손절이 더
  늦게 걸린다. `## 포지션`에 성격을 적고, 진입 지침 5번이 "근거가 약해진 물타기는 작게 내되
  물타기 자체를 금지하지는 않는다"로 확장된다. 지침 5는 크기를 *제한하는* 항목이라 리스크 항목보다 위에 있어도 순서 계약을 깨지
  않는다.
