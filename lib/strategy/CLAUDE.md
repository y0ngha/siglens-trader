...
| `signal-scorer.ts` | Converts analysis results → 0-100 weighted score. Maps trend/sentiment/signals to component scores, then computes weighted average. `SignalScore.technicalInputs`는 기술 축이 **3입력 중 몇 개로** 계산됐는지 남긴다 — 없는 입력은 조용히 건너뛰므로 점수만으로는 축이 반쪽이 된 것을 알 수 없다(실측: 모델 교체로 지표 신호가 824건 전부 빈 배열이 된 채 한 달). |
| `risk-manager.ts` | Position sizing (fixed ratio based on maxPositionSize/maxTotalExposure), stop loss, take profit. Includes `evaluateExistingPosition()` for dynamic exit based on analysis. `PositionEvaluation.hard` marks exits the AI trade gate must never override (see below). **규칙 5(저항선 근접)와 5b(목표가 근접)는 4.5(분석 익절가)의 폴백이라 `aiTakeProfit`이 없을 때만 발동한다** — 항상 돌던 시절 5번은 실측 99.2%, 5b는 v0.30.0부터(`priceTargets`가 채워지기 시작하자) **100%**의 틱에서 조건이 참이었고 "사자마자 청산"이 됐다. `entry-zone.ts`의 `firstUpsideExit`이 같은 규칙을 미러링한다. |
...
| `entry-zone.ts` | 진입 품질 게이트 둘. (1) 권장 진입 구간 상단: `exceedsEntryZone` / `formatEntryZone`, `ENTRY_ZONE_TOLERANCE` 1% — **상단만** 본다. (2) 손절 여유: `hasStopRoom` / `formatStopRoom`, `MIN_STOP_ROOM` 0.5% — 진입가가 `max(지지선, 분석 손절가)`보다 그만큼 위인가. (3) 손익비: `hasRiskReward` / `riskRewardRatio` / `firstUpsideExit` / `formatRiskReward`, `MIN_RISK_REWARD` 1.5 — (2)의 **대칭**으로, 하방만 보던 것에 상방을 더한다. 보상은 **가장 먼저 서는** 익절 트리거까지다(분석 익절가, 그리고 **분석 익절가가 없을 때만** 저항 밴드 하단 / 목표가 95% 중 최솟값) — 더 먼 목표를 세면 도달하지 못할 이익을 계산하게 된다. 저항선·목표가를 조건부로 세는 이유는 청산 규칙 5·5b가 4.5의 폴백이기 때문이다 — 서지도 않을 트리거를 보상 상한으로 잡으면 손익비가 실제보다 낮게 나온다. 익절 레벨이 전부 진입가 이하면 `0`을 돌려준다(판단 불가인 `null`과 구분). 둘 다 재료가 없으면 통과(fail-open)이고 **매수 전용** — 매도에는 쓰지 않는다(원칙 7). |
...
6. Approaching resistance (±2% **밴드**) or target price (95%, 상한 없음) → take_profit —
   **둘 다 5.5의 폴백이라 `aiTakeProfit`이 없을 때만 발동한다.** `targetPrice`는 가장 가까운
   목표라 장중 분석에서는 현재가 대비 중앙 +0.80%(최대 +4.69%)이고, `* 0.95`는 +5.26% 안쪽이면
   항상 참이다 — 실측 537/537틱. 첫 목표가는 77%의 틱에서 `aiTakeProfit`과 같은 가격이라
   "도달"은 5.5가 근사 없이 이미 잡는다.
