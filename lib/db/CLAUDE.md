...
| `queries.ts` | 26 query helper functions (all take `db: Db` as first param) |
...
| `trades` | Execution history (with reason) |
...
Tested with mocked Drizzle builder chain. 100% coverage on queries.ts.
