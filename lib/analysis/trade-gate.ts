...
const GUIDELINES: Record<TradeGateKind, string[]> = {
    entry: [
        ...
        '4. **지표 컨플루언스는 LLM이 아닌 규칙 엔진의 출력이고 신호 점수에서 가장 큰 가중치를 갖는다. 다른 축과 충돌하면 이쪽에 더 무게를 둬라.**',
        '5. **현재 위치와 키 레벨의 관계.** 현재가가 권장 진입 구간 안인지, 저항 바로 아래인지 지지 위인지를 본다. 같은 점수라도 크기가 달라야 한다. 손절가·익절가가 있으면 손익비를 함께 본다.',
        '6. **기존 포지션.** 이미 종목당 한도의 상당 부분을 채웠다면 추가 매수는 작아야 한다.',
        '7. **당일 손익 여력과 남은 장 시간.** 일일 손실 한도에 근접했다면 신규 리스크를 줄인다. 정규장 마감이 임박했거나 임박한 예정 이벤트(실적 발표 등)가 있으면 크기를 줄인다.',
        '8. **청산 판단은 이번 결정에 없다.** 이번은 진입이므로 `fraction`은 예산 대비 비율이다. 진입이 부담스러우면 0에 가까운 값을 내되, 0은 "이번 틱에 아무것도 사지 않는다"를 뜻한다는 점을 알고 낸다.',
    ],
    exit: [
        ...
        // 진입 4번과 같은 자리(펜스 밖)에 두되 무게는 반대다 — 청산 트리거는 백테스트로
        // 검증된 적이 없으므로(위 `CONFLUENCE_SOURCE_LINE` 주석) 결정적 근거가 될 수 없다.
        '4. **지표 컨플루언스의 청산 트리거는 규칙 엔진 출력이지만 백테스트로 검증되지 않았다. 다른 축과 충돌할 때 결정적 근거로 삼지 마라.**',
        '5. **분석의 신선도.** 기준시각이 오래됐으면 지금 상태를 모른다는 뜻이다. 모르는 상태에서 리스크를 들고 가지 않는다 — 크기를 **키운다.**',
        '6. **당일 손익 여력.** 일일 손실 한도에 근접했다면 남은 리스크를 빨리 줄인다.',
    ],
};
