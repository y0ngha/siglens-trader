...
- **물타기는 게이트에 위임한다.** 감사는 "규칙 엔진이 방향을 구분하지 않는다"를 지적했고 초안은
  ...
  **게이트 프롬프트가 물타기임을 알도록** 바꿨다. 핵심은 모델이 계산으로 얻을 수 없는 사실이다:
  종목당 예산은 `한도 − 현재가 × 보유수량`이라 **가격이 내린 만큼 자동으로 커져 있고**, 그
  증가분은 기회가 아니라 평가액 감소의 결과다. `## 포지션`에 성격을 적고, 진입 지침 5번이
  "예산이 커져 보이는 것을 근거로 크기를 키우지 말되 물타기 자체를 금지하지는 않는다"로
  확장된다. 지침 5는 크기를 *제한하는* 항목이라 리스크 항목보다 위에 있어도 순서 계약을 깨지
  않는다.
